...
const ABI = [
  ...
  "function getAgent(address) view returns (string name, string description, string metadataURI, uint256 reputation, uint256 endorsementCount, uint256 registeredAt, uint256 lastActiveAt, bool isActive)",
  "function getEndorsers(address) view returns (address[])",
  "function getEndorsement(address endorser, address endorsed) view returns (address endorser, address endorsed, uint256 timestamp, string reason)",
    ...
    const edgeResults = await Promise.allSettled(
      nodes.map(async (node) => {
        ...
        for (const endorser of endorsers) {
          if (!addrSet.has(endorser.toLowerCase())) continue;
          try {
